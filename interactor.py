from constants import field_x, field_y, field_r, field_outer_r, field_edge_r, center_r, atom_r, atom_inner_r, field_inner_r, atom_colors, rgb_adjust_a, rgb_adjust_b
import pyautogui
import keyboard
import random
import time
...
class Interactor:
    ...
    def get_centers(self, img):
        ...
        for i in range(num_samples):
            p = np.array(self.polar_to_rect(field_r, 2 * i * math.pi / num_samples))
            if img_bw[p[0]][p[1]] < 255: continue
            # print(img_bw[p[0]][p[1]])
            cnt += 1
            # print(cnt)
            s = datetime.datetime.now()
            ret, img_bw, fmask, rect = cv2.floodFill(img_bw, None, (p[1],p[0]), 64)
            fmask = fmask[1:-1,1:-1]
            ...
            if (max(abs(rect[2] - 2 * atom_r), abs(rect[3] - 2 * atom_r)) <= 3):
                c = (np.round(np.sum(fill_coords, axis=0) / np.shape(fill_coords)[0]))
                # print(c)
                coords.append(c)
        ...
        for i in range(expected):
            ...
            for j in range(len(coords_fixed)):
                ...
                coords_fixed.append([coords[0][0], coords[0][1] + min_angle * i])
        coords_fixed.sort(key=lambda x : x[1])
        coords_fixed = list(map(lambda x : self.polar_to_rect(x[0], x[1]), coords_fixed))
        # print(coords_fixed)
    ...
    def ident_atom(self, img):
        ...
        for i in range(-4, self.r):
            ...
            for channel in range(3):  # Assuming a 3-channel color space like HSV or BGR
                result = cv2.matchTemplate(img[:, :, channel], atom[:, :, channel], cv2.TM_SQDIFF_NORMED)
                # print(result)
    ...
    def play(self):
        ...
        while True:
            ...
            if not (op) or curr_atom == -2:
                ...
                click_coords = [self.polar_to_rect(field_r, click_angle)[1], self.polar_to_rect(field_r, click_angle)[0]]
                print(click_coords)
                # input()
